    ...
    <div @dragenter="preventDefault" @dragover="preventDefault" @drop="imageDropped">
        <div class="srow">
            ...
            <button onclick="document.getElementById('inputImg').click();" class="three columns file-input">
                ...
                <span
                    ><i class="fas fa-file-upload" /> <span>{{ $t('chooseFile') }}</span></span
                >
            </button>
            <button
                class="three columns"
                @click="
                    searchText = gridElement.label[i18nService.getContentLang()];
                    searchInput(0);
                "
            >
            ...
            <button class="three columns" v-show="hasImage" @click="clearImage">
                <i class="fas fa-times" /> <span>{{ $t('clearImage') }}</span>
            </button>
        </div>
        <div class="srow">
            <div class="img-preview offset-by-two four columns">
                <span class="show-mobile" v-show="!hasImage"
                    ><i class="fas fa-image" /> <span>{{ $t('noImageChosen') }}</span></span
                >
                <span class="hide-mobile" v-show="!hasImage"
                    ><i class="fas fa-arrow-down" /> <span>{{ $t('dropImageHere') }}</span></span
                >
                <img v-if="hasImage" id="imgPreview" :src="gridElement.image.data || gridElement.image.url" />
                <div v-if="gridElement.image.author">
                    {{ $t('by') }}
                    <a :href="gridElement.image.authorURL" target="_blank">{{ gridElement.image.author }}</a>
                </div>
            </div>
            <div class="img-preview five columns hide-mobile" v-show="hasImage" style="margin-top: 50px">
                <span
                    ><i class="fas fa-arrow-down" /> <span>{{ $t('dropNewImageHere') }}</span></span
                >
            </div>
        </div>
        <div class="srow">
            ...
            <div class="five columns">
                <input
                    ...
                    type="text"
                    v-model="searchText"
                    @input="searchInput(500, $event)"
                    :placeholder="'SEARCH_IMAGE_PLACEHOLDER' | translate"
                />
            ...
            <div class="four columns" v-if="searchProvider">
                ...
                <select
                    ...
                    @change="
                        searchInput(0);
                        localStorageService.save(EDIT_ELEM_SELECTED_SEARCH_PROVIDER, searchProvider.name);
                    "
                >
        ...
        <div class="srow" v-if="searchProvider && (searchProvider.options || searchProvider.searchLangs)">
            <accordion
                class="offset-by-two ten columns"
                :acc-label="$t('settingsForImageSearch')"
                acc-label-type="span"
            ...
            >
                    ...
                    <select
                        ...
                        @change="
                            ...
                            localStorageService.saveJSON(
                                EDIT_ELEM_SEARCH_LANG_PREFIX + searchProvider.name,
                                searchLang
                            );
                        "
                    >
                        <option :value="null">{{ $t('automaticCurrentLanguage') }}</option>
                        <option v-for="value in searchProvider.searchLangs" :value="value">
                            {{ $t('lang.' + value) }}
                        </option>
                    </select>
                </div>
                <div v-for="option in searchProvider.options">
                    <div class="srow" v-if="option.type === constants.OPTION_TYPES.BOOLEAN">
                        <input
                            :id="searchProvider.name + option.name"
                            type="checkbox"
                            v-model="option.value"
                            @input="searchInput(0)"
                        />
                        <label :for="searchProvider.name + option.name">{{
                            $t(searchProvider.name + option.name)
                        }}</label>
                    </div>
                    <div class="srow" v-if="option.type === constants.OPTION_TYPES.COLOR">
                        <label class="three columns" :for="searchProvider.name + option.name">{{
                            $t(searchProvider.name + option.name)
                        }}</label>
                        <span class="two columns" v-if="!option.value">(transparent)</span>
                        <input
                            ref="colorInput"
                            :class="option.value ? 'two columns' : ''"
                            :style="!option.value ? 'height: 1px; width: 1px; opacity: 0' : ''"
                            :id="searchProvider.name + option.name"
                            type="color"
                            v-model="option.value"
                            @input="searchInput(0)"
                        ...
                        />
                        <!-- just for styling reasons in order to keep position of other elements-->
                    ...
                    <div class="srow" v-if="option.type === constants.OPTION_TYPES.SELECT_COLORS">
                        ...
                        <div class="nine columns colorSelector">
                            <div class="inline">
                                <button
                                    :aria-label="$t('noneSelected')"
                                    @click="
                                        option.value = undefined;
                                        afterColorChange();
                                        searchInput(0);
                                    "
                                    :aria-selected="option.value === undefined"
                                >
                                    {{ $t('noneSelected') }}
                                </button>
                            </div>
                            <div v-for="(color, index) in option.colors" class="inline">
                                <button
                                    :aria-label="option.options[index]"
                                    :title="option.colors[index]"
                                    @click="
                                        option.value = option.options[index];
                                        afterColorChange();
                                        searchInput(0);
                                    "
                                    :aria-selected="option.options[index] === option.value"
                                    :style="`background-color: ${backgroundColor}; border-color: ${borderColor}; border-width: 5px; border-style: solid;`"
                                ></button>
                            </div>
                        </div>
                    </div>
            ...
            <div class="offset-by-two ten columns">
                <div v-for="imgElement in searchResults" class="inline">
                    <img
                        v-if="imgElement.url"
                        :src="imgElement.url"
                        @click="gridElement.image = imgElement"
                        :title="$t('byAuthor', [imgElement.author])"
                        width="60"
                        height="60"
                        class="inline img-result"
                        role="button"
                    />
                </div>
                ...
                <span v-show="searchLoading"
                    ><i class="fas fa-spinner fa-spin"></i> <span>{{ $t('searching') }}</span></span
                >
                <span v-show="searchError"
                    ><i class="fas fa-times"></i> <span>{{ $t('searchFailedMaybeNotConnectedToInternet') }}</span></span
                >
                <span v-show="!searchError && !searchLoading && searchResults && searchResults.length === 0">
                    <span
                        ><b>{{ $t('noSearchResults') }}</b></span
                    >
                </span>
            </div>
        </div>
...
export default {
    ...
    computed: {
        hasImage: function () {
            return (
                this.gridElement &&
                this.gridElement.image &&
                (this.gridElement.image.data || this.gridElement.image.url)
            );
        }
    },
    data: function () {
    ...
    methods: {
            ...
            if (base64.length > 50 * 1024) {
                imageUtil.convertBase64(base64, 2 * thiz.elementW).then((newData) => {
                    if (newData.length < base64.length) {
                        log.info(
                            `converted image from ${Math.round(base64.length / 1024)}kB to ${Math.round(
                                newData.length / 1024
                            )}kB`
                        );
                        ...
                        log.info(
                            `converting resulted in bigger image (${Math.round(
                                newData.length / 1024
                            )}kB), using old image with ${Math.round(base64.length / 1024)}kB`
                        );
        ...
        searchInput(debounceTime, event) {
            ...
            util.debounce(function () {
                thiz.searchProvider.service
                    .query(thiz.searchText, thiz.searchProvider.options, thiz.searchLang)
                    .then((resultList) => {
    ...
    mounted() {
        ...
        let currentSearchProviderName = hasSearchProvider
            ? this.gridElement.image.searchProviderName
            : localStorageService.get(EDIT_ELEM_SELECTED_SEARCH_PROVIDER);
        let currentSearchOptions = hasSearchProvider ? this.gridElement.image.searchProviderOptions : null;
        this.searchProvider =
            this.searchProviders.filter((provider) => provider.name === currentSearchProviderName)[0] ||
