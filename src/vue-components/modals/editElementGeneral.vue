<template>
    <div class="container-fluid px-0">
        <div class="row">
            ...
            <div class="col-sm-3">
                <button @click="$emit('searchImage')" class="col-12" :title="$t('searchForImages')">
                    <i class="fas fa-search" /> {{ $t('searchForImages') }}
                </button>
            </div>
        </div>
        <div class="row">
        ...
        <div class="srow mt-5">
            <div class="ten columns">
                <div>
                    ...
                    <button
                        class="inline"
                        @click="
                            gridElement.backgroundColor = null;
                            backgroundColor = metadata.colorConfig.elementBackgroundColor;
                        "
                    >
                        {{ $t('clear') }}
                    </button>
                ...
                <div>
                    <label for="borderColor">{{ $t('customBorderColor') }}</label>
                    <input
                        class="mx-2"
                        type="color"
                        id="borderColor"
                    ...
                    <button
                        class="inline"
                        @click="
                            gridElement.borderColor = null;
                            borderColor = metadata.colorConfig.elementBorderColor;
                        "
                    >
                        {{ $t('clear') }}
                    </button>
            ...
            <div class="six columns">
                <a
                    href="javascript:;"
                    v-if="gridElement.colorCategory && gridElement.backgroundColor"
                    @click="gridElement.colorCategory = undefined"
                    >{{ $t('disableColorCategoryToEnableCustomColor') }}</a
                >
            </div>
        </div>
        <div class="srow">
...
export default {
    ...
    methods: {
        ...
        changeBorderColor() {
            this.gridElement.borderColor = this.borderColor;
        }
    },
    mounted() {
        ...
        dataService.getMetadata().then((metadata) => {
            ...
            this.borderColor = this.gridElement.borderColor || metadata.colorConfig.elementBorderColor;
