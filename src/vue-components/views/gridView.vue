    ...
    <div class="box" id="gridView" v-cloak>
        <header class="srow header" role="toolbar" v-if="metadata" v-show="!metadata.fullscreen">
            ...
            <button
                tabindex="34"
                v-show="!metadata.locked"
                @click="MainVue.showSearchModal()"
                class="spaced small"
                :aria-label="$t('fullscreen')"
                :title="$t('searchBtnTitle')"
            >
        ...
        <huffman-input-modal
            v-if="showModal === modalTypes.MODAL_HUFFMAN"
            @close="
                showModal = null;
                reinitInputMethods();
            "
        ...
        />
        <scanning-modal
            v-if="showModal === modalTypes.MODAL_SCANNING"
            @close="
                showModal = null;
                reinitInputMethods();
            "
        />
        <sequential-input-modal
            v-if="showModal === modalTypes.MODAL_SEQUENTIAL"
            @close="
                showModal = null;
                reinitInputMethods();
            "
        ...
        <div
            ...
            v-show="
                viewInitialized &&
                gridData.gridElements &&
                gridData.gridElements.length === 0 &&
                (!globalGridData || globalGridData.gridElements.length === 0)
            "
        >
        ...
        <div
            ...
            v-show="
                gridData.gridElements &&
                (gridData.gridElements.length > 0 || (globalGridData && globalGridData.gridElements.length > 0))
            "
        >
    ...
    methods: {
        ...
        initInputMethods(continueRunningMethods) {
            ...
            if (inputConfig.dirEnabled) {
                thiz.directionInput = DirectionInput.getInstanceFromConfig(
                    inputConfig,
                    '.grid-item-content:not([data-empty="true"])',
                    'scanFocus',
                    selectionListener
                );
        ...
        reloadFn(event, updatedIds, updatedDocs, deletedIds) {
            ...
            let hasUpdatedGlobalGrid =
                updatedDocs.filter((doc) => this.metadata && doc.id === this.metadata.globalGridId).length > 0;
            this.updatedMetadataDoc =
                updatedDocs.filter((doc) => vueApp.metadata && doc.id === vueApp.metadata.id)[0] ||
                this.updatedMetadataDoc;
            if (updatedGridDoc) {
            ...
            if (
                this.updatedMetadataDoc &&
                JSON.stringify(this.metadata.colorConfig) !== JSON.stringify(this.updatedMetadataDoc.colorConfig)
            ) {
                this.backgroundColor = this.updatedMetadataDoc.colorConfig.gridBackgroundColor;
                this.borderColor = this.updatedMetadataDoc.colorConfig.gridBorderColor;
                vueApp.reload();
            }
    ...
    mounted: function () {
        ...
        dataService
            ...
            .then((savedMetadata) => {
                ...
                this.borderColor = metadata.colorConfig.gridBorderColor;
                metadata.lastOpenedGridId = this.gridId;
                metadata.locked =
                    metadata.locked === undefined
                        ? urlParamService.isDemoMode() && dataService.getCurrentUser() === constants.LOCAL_DEMO_USERNAME
                        : metadata.locked;
                if (metadata.locked) {
                    $(document).trigger(constants.EVENT_UI_LOCKED);
                }
                metadata.fullscreen =
                    metadata.fullscreen === undefined
                        ? urlParamService.isDemoMode() && dataService.getCurrentUser() === constants.LOCAL_DEMO_USERNAME
                        : metadata.fullscreen;
